...
namespace Vehicles04.API.Data
{
    public class SeedDb
    {
        ...
        private async Task CheckUserAsync(string document, string firtsName, string lastName, string email, string phoneNumber, string address, UserType userType)
        {
            ...
            if(user == null)
            {
                user = new User
                {
                    Address = address,
                    Documet = document,
                    DocumetType = _context.DocumetTypes.FirstOrDefault(x => x.Description == "Cédula"),
                    Email = email,
                    FirstName = firtsName,
                    LastName = lastName,
        ...
        {
            if (!_context.DocumetTypes.Any())
            {
                _context.DocumetTypes.Add(new DocumetType { Description = "Cédula" });
                _context.DocumetTypes.Add(new DocumetType { Description = "Tarjeta de Identidad" });
                _context.DocumetTypes.Add(new DocumetType { Description = "DNI" });
                _context.DocumetTypes.Add(new DocumetType { Description = "Pasaporte" });
                await _context.SaveChangesAsync();
            }
        }
